...
export async function getAllData(req, res, next) {
  try {
    logger.info("Rout: /api/v1/user, Method: get")
    ...
    logger.error(`Rout: /api/v1/user, Method: get, Erorr: ${error.message}`)
...
export async function searchData(req, res, next) {
  try {
    logger.info("Rout: /api/v1/search, Method: get")
...
export async function getById(req, res, next) {
  try {
    logger.info("Rout: /api/v1/id, Method: get")
...
export async function createData(req, res, next) {
  try {
    logger.info("Rout: /api/v1/create, Method: post")
    ...
    logger.error(`Rout: /api/v1/user, Method: post, Erorr: ${error.message}`)
    next(error);
  }
}
export const activeUser = async(req, res, next) =>{
  try {
    logger.info("Rout: /api/v1/avtivate, Method: post")
    const {id} = req.params
    ...
    logger.error(`Rout: /api/v1/avtivate, Method: post, Erorr: ${error.message}`)
    next(error)
  }
}
...
export async function loginData(req, res, next) {
  try {
    logger.info("Rout: /api/v1/login, Method: post")
    ...
    if(!oldUser.is_active){
      return res.status(400).send({
        message: "registration isn't done"
      })
    }
    ...
    logger.error(`Rout: /api/v1/login, Method: post, Erorr: ${error.message}`)
    next(error);
  }
}
...
export async function updateData(req, res, next) {
  try {
    logger.info("Rout: /api/v1/id, Method: put")
    ...
    logger.error(`Rout: /api/v1/id, Method: put, Erorr: ${error.message}`)
...
export async function removeData(req, res, next) {
  try {
    logger.info("Rout: /api/v1/id, Method: delete")
    const data = await User.findById(req.params.id);
    const fileUrl = path.join(process.cwd(), "public", "uploads", data.image);
    fs.unlinkSync(fileUrl);
    ...
    logger.error(`Rout: /api/v1/id, Method: delete, Erorr: ${error.message}`)
    next(error);
  }
}
...
export async function refreshToken(req, res, next) {
  try {
    logger.info("Rout: /api/v1/reftesh, Method: get")
    ...
    logger.error(`Rout: /api/v1/refresh, Method: get, Erorr: ${error.message}`)
    next(error);
  }
}
...
export async function checkOtp(req, res, next) {
  try {
    logger.info("Rout: /api/v1/check-otp, Method: post")
    const { otp } = req.body;
    ...
    logger.error(`Rout: /api/v1/check-otp, Method: post, Erorr: ${error.message}`)
    next(error);
  }
}
...
export async function restorePassword(req, res, next) {
  try {
    logger.info("Rout: /api/v1/restorePassword, Method: post")
    ...
    logger.error(`Rout: /api/v1/restorePassword, Method: post, Erorr: ${error.message}`)
    next(error);
  }
}
